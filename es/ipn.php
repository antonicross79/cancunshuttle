...
// Cabeceras para el correo
	...
	$header.= "From: CancunShuttleAirport<no-replyr@cancunshuttleairport.com>";
...
if (!$fp) {
	...
	mail($email_destinatario,utf8_decode("CancunShuttleAirport Ticket"), $recibido , $header);
	...
	while (!feof($fp)) {
		...
		mail($email_destinatario,utf8_decode("CancunShuttleAirport Ticket"), $recibido , $header);
			...
			mail($email_destinatario,utf8_decode("CancunShuttleAirport Ticket"),"INVALIDO",$header);
		}
	}fclose ($fp);
}
